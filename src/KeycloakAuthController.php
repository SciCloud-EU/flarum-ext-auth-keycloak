...
namespace SciCloudEU\Auth\Keycloak;
...
class KeycloakAuthController implements RequestHandlerInterface
{
    ...
    public function handle(Request $request): ResponseInterface
    {
        ...
        if (isset($remoteUserArray['roles']) && is_array($remoteUserArray['roles'])) {
            ...
            if ($roleMapping = json_decode($this->settings->get('scicloud-eu-auth-keycloak.role_mapping'), true)) {
        ...
        if ($attributeMapping = json_decode($this->settings->get('scicloud-eu-auth-keycloak.attribute_mapping'), true)) {
   ...
   public function decorateRegistration(Registration $registration, KeycloakResourceOwner $remoteUser): Registration
   {
      ...
      $modifiedUsername = str_replace(['@', '.'], '_', $rawUsername);
      $registration->suggestUsername($modifiedUsername);
  ...
  public function updateInternalIfNeeded(User $user, KeycloakResourceOwner $remoteUser): User
  {
       ...
       if ($this->settings->get('scicloud-eu-auth-keycloak.delegate_avatars')) {
